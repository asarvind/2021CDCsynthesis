...
\end{definition}
%
We consider the new problem of stabilization of a sampled data system
based on previous sample information when the sampling time is
uncertain, so that we can compensate for the loss of information more
robustly.
%
\begin{problem}
We are given $\lambda\in[0,1)$, $\lb$, $\ub$, $A_s$, $B_s$, $A_u$, $B_u$ and $F_c$ such that the matrix $\R{\lb}$ is Schur.  Find a feedback matrix  $F_p\in\reals^{m\times\rb{n+m}}$ acting on the previous state sample and input and also find a $\theta\in\sqb{\rb{\ub-\lb}/2, \rb{\ub-\lb}}$ such that the system is $\lambda$-exponentially stable.
\end{problem}
%
...
evolution based feedback matrix $F_p = F_c\exp\rb{A_c\rb{\lb+\mu}}$ at some
