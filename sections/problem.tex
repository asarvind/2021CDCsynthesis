...
\end{problem}
%
\begin{remark}~\label{rem:counterintuitive}  Let us consider that a system is be stable for sampling times between $\sqb{\lb,\lb+\mu}$ without historical feedback.  We wish to stabilize the system for a large $\ub>\rb{\lb+\mu}$ applying feedback based on previous sample.  The expected sampling time feedback at a time $\rb{t_k+\mu}$ is ${F_c\state{\rb{t_k+\mu}}}$.  As a function of the previous sample $\state{t_k}$, this feedback is $F_c\exp\rb{A_c\rb{\lb+\mu}}\state{t_k}$.  So, when the sample information is missing at $\rb{t_k+\lb+\mu}$, it may seem intuitive to apply feedback $F_c\exp\rb{A_c\rb{\lb+\mu}}\state{t_k}$ at $\rb{\lb+\mu}$ to stabilize the system.   However, other values of $F_p$ may lead to far more robust stabilization as illustrated in the below example.
\end{remark}
%
...
matrix $F_p = F_s\exp\rb{1.888A_c}$ acting on previous sample at
sampling time $1.888$ where $\theta = 1.488$, the system will be unstable
...
no guarantee that such feedback will stabilize the system.
\end{example}
