...
The system is $\lambda$-exponentially stable if there exists a set $\Psi\subseteq\reals^{n+m}$ such that $\max_{z\in\Psi}\norm{z}<\infty$, $\ball{0}{1}\subseteq \Psi$ and $\forall \tau\in\sqb{\lb,\ub}$ $\R{\tau}\Psi\subseteq \lambda\Psi$.
\end{lemma}
%
\begin{proof}
...
\end{proof}
%
Therefore, to stabilize the system, we can find an $F_p$, $\theta$ and a set $\Psi$ satisfying the conditions of Lemma~\ref{lem:invstability}.  The choice of the set $\Psi$ affects the robustness of our solution, {\em i.e.}, the $\ub$ for which stabilization can be achieved.  It has been previously shown that using the eigenstructure of the system dynamics in a set representation called complex zonotope~\cite{adimoolam2016using}, stability can be verified for larger ranges of $\ub$ on benchmark examples, compared to other set-based methods. Hence, in this paper, our choice for the $\Psi$ is a complex zonotope and we extend its application to stabilize the system, {\em i.e.}, find $F_p$ and $\theta$.  Firstly, we discuss necessary operations on a complex zonotope in the following section.
%
\subsection{Complex Zonotope and Set Operations}
\input{sections/czoperations}
%
\subsection{Stabilization using complex zonotopes}
