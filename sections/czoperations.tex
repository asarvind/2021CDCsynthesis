A simple zonotope introduced in~\cite{girard2005reachability} is a Minkowski sum of line segments in the real euclidean space described as linear combination of real-valued vectors, called generators, with bounded combining coefficients.  However, the eigenstructure of the dynamics, which can be complex valued, is closely related to finding invariants or contractive sets as discussed in~\cite{adimoolam2016using,adimoolam2016template,adimoolam2018calculus} (also see below Theorem~\ref{thm:eig}).  Since real zonotopes can not incorporate complex valued eigenvectors as generators, we extended real zonotopes to complex zonotopes in~\cite{adimoolam2016using,adimoolam2016template,adimoolam2018calculus}.  A complex zonotope is a linear combination of complex-valued vectors with complex combining coefficients bounded inside circles of the complex plane.  Complex zonotopes are geometrically more expressive than real zonotopes, as they can be Minkowski sum of some embedded ellipses in addition to line segments.   They can capture the contraction of of a set along the eigenvectors of the dynamics as stated in Theorem~\ref{thm:eig}.
%
\begin{definition}
...
\end{definition}
%
The following theorem, which was previously discussed in~\cite{adimoolam2016using,adimoolam2017template} illustrates the relation between the eigenstructure of the system and finding contractive complex zonotopes.
%
...
Let us consider a complex zonotope $\cz{P}{c}{s}$ and a real matrix $A$.  Then,
%
\begin{align*}
    & A\cz{P}{c}{s} = 
...
\end{proof}
%
The algorithm we propose later to stabilize the system involves checking inclusion of a set of complex zonotopes, obtained by applying a sequence of transformations, inside another original complex zonotope (see Lemma~\ref{lem:invstability}).  In this regard, we define a set of complex zonotopes whose templates lie in the neighborhood of a given template as follows.  Let us consider a complex matrix $Q$, a real matrix with positive entries $\Upsilon\in\reals_{\geq 0}^{\rows{Q}\times\cols{Q}}$ and a real vector $\rho\in\reals^{\cols{e}}$ where $e$ is a real vector.  Then we use the following notation.
%
\begin{align*}
    & Q\errplus\Upsilon = \set{Q+\widehat{Q}\st{\abs{\widehat{Q}}\leq \Upsilon}},~~e\errplus\Upsilon = \set{e+u\st{\abs{u}\leq \rho}}\\
    & \cz{Q\errplus\Upsilon}{e\errplus\rho}{r} \\
    & = \set{\cz{Q+\widehat{Q}}{e+u}{r}~\st{\abs{\widehat{Q}}\leq \Upsilon,~\abs{u}\leq \rho}}
\end{align*}
%
The following new relation introduced in this paper is a sufficient condition for checking the inclusion of the above set of complex zonotopes inside a complex zonotope, which is proved later in Lemma~\ref{lem:inclusion}.  It is an extension of the inclusion checking between two complex zonotopes discussed in~\cite{adimoolam2018calculus} (see Theorem 2.3.8) to check inclusion of the above set of complex zonotopes inside a complex zonotope.
%
...
%
\begin{proof}
Let us assume that  $ \cz{Q\errplus \Upsilon}{e\errplus \rho}{r}\order \cz{P}{c}{s}$ is true.  Hence, there exist complex matrices $X,\Delta$ and complex vector $y$ such that all the equations~(\ref{eqn:inc1})-(\ref{eqn:inc2}) are true.
%
% \begin{align*}
...
\begin{align*}
 ...
 &~\text{\% as $\abs{\alpha}_j\leq 1$, $\abs{\widehat{Q}}\leq \Upsilon$, $\abs{u}\leq \rho$  }\\
 & \leq \abs{y}_i + \abs{\pinv{P}}\rho  \\
 & ~~~~+ \sum_{j=1}^{\cols{X}} \rb{\abs{X}_{ij}+\rb{\abs{\pinv{P}}\Upsilon\diag{r}}_{ij}}\\ 
 & \text{\% using (\ref{eqn:inc1})-(\ref{eqn:inc2})}\\
